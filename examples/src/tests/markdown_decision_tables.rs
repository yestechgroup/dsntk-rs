...
| 4 | (years and months duration(Date of Birth, today()).years) >= 65  | "senior"  | "Senior: 65+ years" |"#;
        ...
        assert!(decision_table.rules[0].input_entries[0].text.contains("years and months duration"), "Should preserve FEEL duration function");
        ...
        assert!(decision_table.rules[1].input_entries[0].text.contains("years and months duration"), "Should preserve FEEL duration function");
        ...
        assert!(decision_table.rules[2].input_entries[0].text.contains("years and months duration"), "Should preserve FEEL duration function");
        ...
        assert!(decision_table.rules[3].input_entries[0].text.contains("years and months duration"), "Should preserve FEEL duration function");
...
| 2 |   B   |   2    |"#;
        ...
        assert!(matches!(decision_table_unique.hit_policy, dsntk_recognizer::HitPolicy::Unique), "Should have Unique hit policy");
...
| 2 |   A   |   2    |"#;
        ...
        assert!(matches!(decision_table_first.hit_policy, dsntk_recognizer::HitPolicy::First), "Should have First hit policy");
    }
...
| 3 |   "probation"   |     90    |    "Compliant"    |"#;
        ...
        assert_eq!(value_null!("no rules matched, no output value defined"), result4, "trial employment with 89 days should have no matching rule");
    }
...
| 3 | "Private"  |   -  |   0.05   |"#;
        ...
        let evaluator4 = build_decision_table_evaluator(&scope4, &model_decision_table).expect("Failed to build evaluator");
        let result4 = evaluator4(&scope4);
        assert_eq!(value_null!("no rules matched, no output value defined"), result4, "Government customer should have no matching rule");
    }
